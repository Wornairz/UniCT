...
int LCS(string s1, int l1, string s2, int l2)
{
    ...
    for (int i = 1; i <= l1; i++)
    {
        for (int j = 1; j <= l2; j++)
        {
            if (s1[i - 1] == s2[j - 1])          //se i due caratteri in considerazione sono uguali le stringhe hanno un altro carattere uguale
                c[i][j] = c[i - 1][j - 1] + 1;   //quindi aggiungiamo 1 al valore della matrice rispetto al valore senza i due caratteri uguali
